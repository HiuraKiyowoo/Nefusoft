  ...
  <div className="animate-pulse w-full">
    ...
    <div className="bg-[#16161a] rounded-sm border border-white/5 p-6 flex flex-col md:flex-row gap-6 relative overflow-hidden mb-10 shadow-xl">
      ...
      <div className="flex flex-col flex-1 w-full gap-4 z-10 items-center md:items-start mt-2">
        ...
        <div className="flex gap-2 mb-4"><div className="w-12 h-5 bg-white/10 rounded-sm"></div><div className="w-16 h-5 bg-white/10 rounded-sm"></div></div>
...
// ─── Helper: normalise detail dari API baru ──────────────────────────────────
// /detail?url=slug  → { data: [{ judul, cover, sinopsis, chapter:[{ch, url}], ... }] }
...
// ─── Helper: normalise satu item latest untuk rekomendasi ───────────────────
...
const Watch = () => {
  // slug sekarang = url slug dari API, contoh: "majo-tabitabi-sub-indo"
  const { slug, episode } = useParams();
  const navigate = useNavigate();

  const [anime, setAnime]                   = useState(null);
  const [episodes, setEpisodes]             = useState([]);
  const [currentEpId, setCurrentEpId]       = useState(null); // = episode url slug
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      ...
      try {
        // ── /detail?url=slug ────────────────────────────────────────────────
        // Pastikan slug punya trailing slash sesuai format API
        const urlParam = slug.endsWith('/') ? slug : `${slug}/`;
        const res  = await fetch(`/api/detail?url=${urlParam}`).then(r => r.json());
        const raw  = res.data || res;
        const norm = normaliseDetail(raw);

        if (norm) {
          setAnime(norm);
          setEpisodes(norm.episode_list || []);
          updateMetaTags(
            `Tonton ${norm.title} - NefuSoft`,
            norm.synopsis ? norm.synopsis.substring(0, 150) + '...' : 'Streaming anime subtitle Indonesia gratis.',
            norm.image_poster || norm.image_cover
          );
        }

        // ── Rekomendasi dari /latest ─────────────────────────────────────
        const recRes  = await fetch('/api/latest?page=1').then(r => r.json());
        const recRaw  = Array.isArray(recRes) ? recRes : (recRes.data || []);
        setRecommendations(recRaw.map(normaliseLatest).slice(0, 5));
  ...
  // ─── Set episode aktif saat episodes/route berubah ────────────────────────
  useEffect(() => {
    if (episodes.length > 0) {
      // `episode` param dari URL = index ep (angka), cari berdasarkan index
      const target = episode
        ? episodes.find(e => e.index.toString() === episode)
        : episodes[episodes.length - 1]; // default: episode terbaru (index terakhir)
      if (target && target.id !== currentEpId) setCurrentEpId(target.id);
    }
  ...
  useEffect(() => {
    ...
    const fetchEpisode = async () => {
      ...
      try {
        // ── /episode?url=ep-slug&reso=720p ──────────────────────────────
        const epUrl = currentEpId.endsWith('/') ? currentEpId : `${currentEpId}/`;
        const res   = await fetch(`/api/episode?url=${epUrl}&reso=720p`).then(r => r.json());

        // Response: { data: [{ stream:[{link, reso}], reso:["360p","480p","720p"] }] }
        const epData  = res.data?.[0] || res.data || res;
        const streams = epData?.stream || [];
        const resos   = epData?.reso   || [];

        // Filter hanya mp4 direct
        const mp4Servers = streams
          .filter(s => s.link && s.link.split('?')[0].endsWith('.mp4'))
          .map((s, i) => ({ id: i, link: s.link, quality: s.reso || resos[i] || '720p' }));
        ...
        if (unique.length > 0) {
          setSelectedServer(
            unique.find(s => s.quality === '720p') ||
            unique.reduce((p, c) => (parseInt(c.quality) > parseInt(p.quality) ? c : p))
          );
        } else {
          setSelectedServer(null);
        }
      } catch (e) { console.error('fetchEpisode error:', e); }
  ...
  const resetControlsTimeout = () => {
    ...
    controlsTimeoutRef.current = setTimeout(() => {
      if (!isDraggingRef.current) { setShowControls(false); setShowResolutions(false); setShowSpeeds(false); }
    }, 3000);
  };
  ...
  const handleVideoAreaClick = (e) => {
    ...
    else { if (controlsTimeoutRef.current) clearTimeout(controlsTimeoutRef.current); setShowControls(false); setShowResolutions(false); setShowSpeeds(false); }
  };
  ...
  const togglePlay = () => {
    ...
    if (videoRef.current.paused) {
      ...
      videoRef.current.play().then(() => { setIsPlaying(true); setHasStarted(true); resetControlsTimeout(); }).catch(() => { setIsPlaying(false); setIsBuffering(false); });
    } else {
      videoRef.current.pause(); setIsPlaying(false); resetControlsTimeout();
    }
  };
  ...
  const handleTimeUpdate = () => {
    ...
    setDuration(dur);
    if (!isDraggingRef.current) { setCurrentTime(cur); setProgress((cur / dur) * 100); }
  };

  const handleSeekBegin  = () => { isDraggingRef.current = true; resetControlsTimeout(); };
  const handleSeekChange = (e) => { const val = parseFloat(e.target.value); setProgress(val); setCurrentTime((val / 100) * duration); resetControlsTimeout(); };
  ...
  const handleProgressInteraction = (e) => {
    ...
    if (canvasRef.current) { const ctx = canvasRef.current.getContext('2d', { willReadFrequently: true }); ctx.fillStyle = '#000000'; ctx.fillRect(0, 0, 160, 90); }
    clearTimeout(hoverTimeoutRef.current);
    hoverTimeoutRef.current = setTimeout(() => { if (hiddenVideoRef.current && hiddenVideoRef.current.readyState >= 1) hiddenVideoRef.current.currentTime = hoverTime; }, 50);
  };

  const hideProgressPreview = () => setHoverPos(prev => ({ ...prev, show: false }));

  const drawThumbnail = () => {
    try { if (hiddenVideoRef.current && canvasRef.current) { const ctx = canvasRef.current.getContext('2d', { willReadFrequently: true }); ctx.drawImage(hiddenVideoRef.current, 0, 0, 160, 90); } } catch(e) {}
  };
  ...
  const handleSkip = (amount) => {
    ...
    skipTimeout.current = setTimeout(() => { if (videoRef.current) videoRef.current.currentTime += skipBuffer.current; skipBuffer.current = 0; setSeekPopup(null); resetControlsTimeout(); }, 600);
  };
  ...
  const handleResolutionChange = (server) => {
    if (videoRef.current) {
      ...
      setTimeout(() => { if (videoRef.current) { videoRef.current.currentTime = curTime; if (wasPlaying) videoRef.current.play().catch(() => {}); } }, 200);
    }
    setShowResolutions(false); resetControlsTimeout();
  };

  const handleSpeedChange = (speed) => {
    if (videoRef.current) videoRef.current.playbackRate = speed;
    setPlaybackSpeed(speed); setShowSpeeds(false); resetControlsTimeout();
  };
  ...
  // ─── Navigate ke rekomendasi ──────────────────────────────────────────────
  const goDetail = (item) => {
    const s = item.url ? item.url.replace(/\/$/, '') : (item.id || '');
    navigate(`/anime/${s}`);
  };
            ...
            <div className="bg-[#16161a] p-1.5 md:p-2 rounded-sm border border-white/5 mb-4 shadow-2xl relative">
              ...
              >
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                  {(() => {
                    if (isEpLoading || (!isVideoReady && !hasStarted)) return (
                    ...
                    if (isBuffering && hasStarted) return (
                  ...
                  {selectedServer && !isEpLoading ? (
                    <>
                      <video
                        ...
                        onLoadedMetadata={() => { if (videoRef.current) { setDuration(videoRef.current.duration); setIsVideoReady(true); } }}
                        onCanPlay={() => {
                          setIsVideoReady(true); setShowControls(true); resetControlsTimeout();
                          if (autoNext && !hasStarted) {
                            videoRef.current.play().then(() => { setIsPlaying(true); setHasStarted(true); resetControlsTimeout(); }).catch(() => {});
                          }
                        }}
                        onTimeUpdate={handleTimeUpdate}
                        onWaiting={() => setIsBuffering(true)}
                        onPlaying={() => setIsBuffering(false)}
                        onEnded={() => { setIsPlaying(false); if (autoNext && epIndex > 0) handleNext(); }}
                        autoPlay={autoNext}
                        playsInline
                        crossOrigin="anonymous"
                      />
                      <video ref={hiddenVideoRef} src={getProxyUrl(selectedServer.link)} className="hidden" crossOrigin="anonymous" onSeeked={drawThumbnail} muted />
                    </>
                  ) : !isEpLoading && (
                ...
                <div className={`absolute inset-0 transition-opacity duration-300 flex flex-col justify-between z-30 pointer-events-none ${showControls ? 'opacity-100 bg-black/40' : 'opacity-0'}`}>
                  ...
                  <div className={`w-full flex flex-col pointer-events-auto bg-gradient-to-t from-black/80 to-transparent pt-16 relative transition-opacity ${isVideoReady ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={e => e.stopPropagation()}>
                    <div className="flex justify-between items-center px-4 md:px-6 pb-5">
                      ...
                      <div className="flex items-center gap-4 md:gap-6">
                        ...
                        <div className="relative">
                          <button onClick={() => { setShowSpeeds(!showSpeeds); setShowResolutions(false); }} className="text-[11px] md:text-xs font-black hover:text-[#F6CF80] transition-colors drop-shadow-md">{playbackSpeed}x</button>
                          {showSpeeds && (
                            <div className="absolute bottom-full right-0 mb-4 bg-[#16161a] border border-white/5 shadow-2xl p-1 flex flex-col min-w-[80px] z-50 rounded-sm">
                              {[0.5, 1, 1.25, 2].map(s => (
                                <button key={s} onClick={() => handleSpeedChange(s)} className={`text-xs font-bold px-3 py-2 text-left ${playbackSpeed === s ? 'text-[#F6CF80] bg-white/5' : 'text-white/70 hover:bg-white/5'}`}>{s}x</button>
                              ))}
                            </div>
                          )}
                        </div>
                        {/* Resolution */}
                        <div className="relative">
                          <button onClick={() => { setShowResolutions(!showResolutions); setShowSpeeds(false); }} className="text-[11px] md:text-xs font-black hover:text-[#F6CF80] transition-colors uppercase drop-shadow-md">
                          ...
                          {showResolutions && (
                            <div className="absolute bottom-full right-0 mb-4 bg-[#16161a] border border-white/5 shadow-2xl p-1 flex flex-col min-w-[100px] z-50 rounded-sm">
                              {servers.map(s => (
                                <button key={s.id} onClick={() => handleResolutionChange(s)} className={`text-xs font-bold px-3 py-2 text-left ${selectedServer?.id === s.id ? 'text-[#F6CF80] bg-white/5' : 'text-white/70 hover:bg-white/5'}`}>{s.quality}</button>
                              ))}
                            </div>
                          )}
                        </div>
                    ...
                    >
                      ...
                      <input type="range" ref={progressContainerRef} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-20 m-0" min="0" max="100" step="0.1" value={progress || 0}
                        onMouseDown={handleSeekBegin} onTouchStart={handleSeekBegin} onMouseUp={handleSeekCommit} onTouchEnd={handleSeekCommit} onChange={handleSeekChange} />
                ...
                <div className="relative z-10 p-6 flex flex-col md:flex-row gap-6 w-full items-center md:items-start">
                  ...
                  <div className="flex flex-col flex-1 w-full text-center md:text-left">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8 text-[10px] md:text-xs">
                      ...
                      <div className="flex border-b border-white/5 pb-2"><span className="text-white/40 font-black uppercase tracking-widest w-24 text-left">Genre</span><span className="text-[#F6CF80] font-bold flex-1 text-right md:text-left">{anime.genre}</span></div>
          ...
          <div className="mb-12">
            ...
            <div className="flex flex-col gap-3">
              {recommendations.map((a) => (
                <div key={a.id} onClick={() => goDetail(a)} className="group cursor-pointer relative h-20 md:h-24 rounded-sm bg-[#16161a] border border-white/5 flex items-center px-4 overflow-hidden transition-transform active:scale-[0.98]">
                  <div className="absolute right-0 top-0 bottom-0 w-1/2 md:w-1/3 z-0">
                    ...
                    <img src={a.image_cover || a.image_poster} referrerPolicy="no-referrer" className="w-full h-full object-cover opacity-40 group-hover:opacity-80 transition-opacity duration-500" alt={a.title} />
                  </div>
                  <div className="relative z-20 flex items-center gap-4 w-full">
                    <img src={a.image_poster} referrerPolicy="no-referrer" className="w-12 md:w-16 aspect-[3/4.2] object-cover rounded-sm shadow-lg group-hover:scale-105 transition-transform" alt={a.title} />
