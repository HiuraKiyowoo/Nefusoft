    ...
    {[...Array(6)].map((_, i) => (
      <div key={i} className="flex items-stretch gap-4 md:gap-6 relative w-full mb-4">
        <div className="w-12 md:w-16 shrink-0 pt-4 flex justify-end">
          <div className="w-10 h-3 bg-[#16161a] rounded-sm relative overflow-hidden">
            <Shimmer />
          </div>
        </div>
...
const ScheduleCard = ({ a, onClick, index }) => {
  ...
  return (
    <div
      className={`flex items-stretch gap-4 md:gap-6 relative group w-full mb-4 transition-all duration-700 ease-out ${
        isVisible
    ...
    >
      ...
      <div className="flex-1 py-4 w-full min-w-0">
        <div
          onClick={onClick}
          className="relative rounded-xl flex p-3 md:p-4 gap-4 md:gap-5 cursor-pointer transition-all active:scale-[0.98] bg-[#16161a] border border-white/5 hover:border-[#F6CF80]/30 group/card overflow-hidden shadow-xl"
        >
          <img
            src={a.image_poster}
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0c] font-nunito selection:bg-[#F6CF80] selection:text-black pb-24">
      ...
      <div className="pt-24 max-w-2xl mx-auto px-4">
        <h2 className="text-white font-black text-xl text-center mb-8 italic tracking-tighter">
          JADWAL TAYANG
        </h2>
        ...
        <div className="flex justify-between items-center mb-8 px-2 border-b border-white/5 pb-4">
          {weekDates.map((w) => (
            ...
            >
              ...
              <div
                className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center text-xs md:text-sm font-black transition-all ${
                  selectedDay === w.key
                    ? 'bg-[#F6CF80] text-black shadow-lg shadow-[#F6CF80]/30'
                    : 'text-white/60 group-hover:bg-white/5'
                }`}
              >
                {w.date}
              </div>
              <div
                className={`w-1 h-1 rounded-full transition-opacity ${
                  w.isToday ? 'bg-[#F6CF80] opacity-100' : 'opacity-0'
                }`}
              ></div>
            </div>
          ))}
        ...
        <div className="flex flex-col">
          {isLoading ? (
            ...
            <div className="py-20 flex flex-col items-center justify-center text-center">
              <svg
                className="w-12 h-12 text-white/5 mb-4"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
